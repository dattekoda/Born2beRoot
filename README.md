...
### SSHとは
> Secured Shellの略。ネットワーク越しに、安全に「シェル」（コマンド操作環境）を使ったり、ファイル転送をしたりするためのプロトコル（通信規約）のこと。
> 暗号化された通信で、（RSAなどの）公開鍵暗号方式をまずはじめに使い、その後は対象鍵暗号（AESなど）で通信をまるごと暗号化する。これによって、中間車攻撃や盗聴を防ぐことができる。
...
`sudo grep Port /etc/ssh/ssed_config`でポート設定が合っているかを確認する。
...
> しかし、iptablesはその自由度と機能の豊富さから、設定を行うのに専門の知識が必要になる。少なくともいきなりiptablesをりようして自由自在に設定を行うことは困難となる。なぜならその設定は根本的な部分から行われるため非常に複雑になるから。

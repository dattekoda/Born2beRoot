...
### DebianとRockyの違いは？
> Debianはコミュニティ主体で開発された非営利プロジェクト。自由度の高いパッケージ管理ができてよりユーザーフレンドリーなUIを持ち、豊富なライブラリとファイルシステムと環境に恵まれる。  
RockyはRed Hat系。Red Hat Enterprise Linuxは企業向けのDistributionでサポートが保証されているものの有料という欠点がある。かつて、Red Hat社はRHELの無料版としてCentOSを無料版として公開していた。
Red Hat社がCentOSを非公開にしたとき、大きな反感を買った。そして、当時Red Hatで働いていたいくつかのメンバーがRocky社を離れて、RHELと100%バグが一致するOSを公開するに至った。
